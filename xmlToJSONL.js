...
function decodeIfString(value) {
  return typeof value === 'string' ? he.decode(value) : value
}
    ...
    jsonObj.CARDS.SET.CARD.forEach(card => {
      ...
      if (trophies) {
        if (Array.isArray(trophies)) {
          ...
          fmtTrophies[trophies['@_type']] = parseInt(trophies['#text'])
        }
      }
      ...
      const formattedCardData = {
        ID: card.ID,
        NAME: card.NAME,
        TYPE: decodeIfString(card.TYPE),
        MOTTO: decodeIfString(card.MOTTO),
...
;['1', '2', '3', '4'].forEach(num => {
  parseXMLtoJSON(`data/cardlist_S${num}.xml`, `cardlist_S${num}.jsonl`)
})
